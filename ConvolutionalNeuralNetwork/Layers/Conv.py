...
class Conv(BaseLayer):
    ...
    def MyZeroPad(self, x, pad_before,pad_after):
    ...
    def forward(self, input_tensor):
        ...
        padded_input = self.MyZeroPad(input_tensor,pad_before,pad_after)
                ...
                if not self.is1D:
                    for x in range(n_X):
                        for y in range(n_Y):
                            ...
                            output[i, c, y, x] = self.myConv(input_slice, fil, b)
                else:
                    for y in range(n_Y):
                        y_range = (strideY * y, strideY * y + fy)
                        input_slice = padded_img[:,y_range[0]:y_range[1]]
                        output[i, c, y] = self.myConv(input_slice, fil, b)
    ...
    def backward(self,error_tensor):
        ...
        # padding
        input_pat = self.MyZeroPad(self.input_tensor, pad_before,pad_after)
        der_input_pad = self.MyZeroPad(der_input, pad_before,pad_after)
